...
void set_Command(path_link root_nodes, int *uninit){
    ...
    int i, n_comp;/* Stores the number of components from the input path*/
    Path_item valued_new_path_item;
    path_link current = root_nodes, current_head,current_parent = NULL, valued_new_path;
    input_double_value(&name, &value);/*Getting input parameters*/
    ...
    else { /*In case the path already existed its value is updated*/
